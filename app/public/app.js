...
(() => {
  ...
  const renderSkus = () => {
    ...
    $('#skuGrid').innerHTML = linhas.map((a, i) => {
      ...
      <div class="sku-card" data-ean="${esc(a.ean)}" style="animation-delay:${i * 60}ms; --sku-color:${SKU_COLOR[a.sku] || 'var(--border-default)'}">
        ...
        <div class="sku-stats">
          <div class="sku-stat"><div class="sku-stat-label">VB Atual</div><div class="sku-stat-value">${a.estoque_ideal ?? '—'}</div></div>
  ...
  const atualizarDica = (card) => {
    ...
    alvo.innerHTML = `${ICONE_DICA}<span><strong>Atenção:</strong> ${esc(texto)}</span>`;
    ...
    try {
      ...
      $('#successDetail').innerHTML = `
        ...
        <span>Status: ${state.bia
          ? '<span class="badge badge-green">Atendido</span>'
          : '<span class="badge badge-warning">Em análise</span>'}</span>
